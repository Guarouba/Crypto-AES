    ...
    for index, value in enumerate(messages):
        ...
        for part in xrange(0, text_len, line_length):
            if part > text_len - line_length:
                ...
                temp = [value[part:part + line_length]]
                print ("     " + str(temp))
...
def root_of_all_decryption():
    ...
    while choice != "end":
        try:
            cipher_nb = int(raw_input("Enter message number: "))

            if (cipher_nb < len(ciphers)) and (cipher_nb >= 0):
                try:
                    crib = input(
                        "Enter your crib (have to be between \" \"): ")  # use input() instead of raw_input() to be able to use "\n"
                    if mode == "cribFollow":
                        tempDecryptedText = get_temp_decrypted_text(cipher_nb)
                        crib = tempDecryptedText + crib
                    temp = []  # len = shortest cipher
                    to_print = []
                    extracte_something_from_the_really_long_mthd(cipher_nb, crib, temp, to_print)
                    print_guess(temp, to_print)
                    print("")
                    choice = raw_input(
                        "Enter the matched position, 'none' for no match, 'switch' to switch to the other decryption mode, or 'end' to quit: ").strip(
                        " ")
                    if choice not in ["none", "end", "switch"]:
                        choice = valid_choice(choice, cipher_nb, crib, temp)
                    elif choice == "switch":
                        switch_mode()
                    print("The current messages are:\n\n")
                    print_messages(current_messages, 128)
            ...
            print("You've entered a wrong number, repeat the process")
        else:

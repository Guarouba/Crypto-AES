...
if __name__ == '__main__':
	...
	while(choice != "end"):
		...
		if((cipherNb < len(ciphers)) and (cipherNb >= 0)):
			...
			try:
				crib = input("Enter your crib (have to be between \" \"): ") #use input() instead of raw_input() to be able to use "\n"
				...
				choice = raw_input("Enter the matched position, 'none' for no match, 'switch' to switch to the other decryption mode, or 'end' to quit: ")

				if(choice != "none" and choice != "end" and choice != "switch"):
					position = int(choice)
					for i in range(len(currentMessages)):
						cribLenght = len(temp[position][i])
						currentMessages[i] = currentMessages[i][:position] + temp[position][i] + currentMessages[i][position+cribLenght:]
